...
class MapOutputTrackerSuite extends FunSuite {
  ...
  test("master register and fetch") {
    ...
    assert(statuses(0).address === new BlockManagerId("hostA", 1000))
    assert(statuses(1).address === new BlockManagerId("hostB", 1000))
    assert(statuses(0).size === size1000)
    assert(statuses(1).size === size10000)
    tracker.stop()
  }
